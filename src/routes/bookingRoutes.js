...
export default async function bookingRoutes(fastify, opts) {
  ...
  fastify.post('/:id/accept', { preHandler: verifyJWT, schema: idParams }, async (request, reply) => {
    const { acceptBooking } = await import('../controllers/bookingController.js');
    return acceptBooking(request, reply);
  });
  fastify.post('/:id/reject', { preHandler: verifyJWT, schema: {
  ...
  fastify.post('/:id/quotes', { preHandler: [verifyJWT, requireRole('artisan')], schema: createQuoteSchema }, createQuote);
